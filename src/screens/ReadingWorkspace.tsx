...
function Button({ label, onPress, colors, quiet, disabled, icon }: { label: string; onPress: () => void; colors: ReadingColors; quiet?: boolean; disabled?: boolean; icon?: IconName }) {
  ...
  return <Pressable accessibilityRole="button" accessibilityLabel={label} onPress={onPress} disabled={disabled} onHoverIn={() => setHovered(true)} onHoverOut={() => setHovered(false)} style={({ pressed }) => [styles.button, { backgroundColor: quiet ? hovered ? colors.line : colors.soft : colors.primary, borderColor: quiet ? colors.line : colors.primary, opacity: disabled ? 0.45 : pressed ? 0.76 : hovered ? 0.9 : 1, transform: [{ scale: pressed ? 0.985 : 1 }] }]}>{icon && <Icon name={icon} size={18} color={quiet ? colors.ink : colors.onPrimary} />}<Text style={[styles.buttonText, { color: quiet ? colors.ink : colors.onPrimary, fontFamily: colors.font }]}>{label}</Text></Pressable>;
}
...
function CardPreview({ card, colors, desktop, open }: { card: ContextCard; colors: ReadingColors; desktop: boolean; open: () => void }) {
  const [hovered, setHovered] = useState(false);
  return <Pressable accessibilityRole="button" accessibilityLabel={`開啟原句字卡 ${card.text}`} onPress={open} onHoverIn={() => setHovered(true)} onHoverOut={() => setHovered(false)} style={({ pressed }) => [styles.contextCard, desktop ? styles.twoColumn : styles.fullWidth, { backgroundColor: colors.card, borderColor: hovered ? colors.blue : colors.line, opacity: pressed ? 0.7 : 1 }]}>
