...
type Tab = 'search' | 'saved' | 'review' | 'settings';
...
function Application() {
  ...
  const speak = (text: string, language: string = data.voice) => {
    void Speech.stop().then(() => Speech.speak(text, { language, rate: 0.85, onError: () => setMessage('裝置暫時無法發音，請確認已安裝對應語音。') })).catch(() => setMessage('裝置暫時無法發音，請稍後再試。'));
  };
  ...
  return <SafeAreaView style={[s.safe, { backgroundColor: colors.bg }]} edges={['top', 'bottom']}>
    <StatusBar style={dark ? 'light' : 'dark'} />
    <View style={[s.frame, { backgroundColor: colors.bg }]}>
      <View style={[s.brandBar, { borderColor: colors.line }]}><View style={s.brand}><Icon name="book-outline" color={colors.ink} size={22} /><Text style={[s.brandText, textColor]}>LexiHarbor</Text></View><Text style={[s.badge, { backgroundColor: colors.soft, color: colors.ink }]}>詞庫試用版</Text></View>
      ...
      {!data.ready ? <View style={s.loading}><ActivityIndicator color="#27664D" /><Text style={mutedColor}>正在讀取學習紀錄…</Text></View> : <ScrollView style={s.main} contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
        ...
        {tab === 'settings' && <>
          ...
          <Text style={[s.sectionTitle, textColor]}>英文發音</Text><View style={s.segment}>{button('美式', () => data.changeVoice('en-US'), data.voice !== 'en-US')}{button('英式', () => data.changeVoice('en-GB'), data.voice !== 'en-GB')}{button('試聽', () => speak('Hello, welcome to LexiHarbor.'), true)}</View><Text style={[s.caption, mutedColor]}>由裝置語音引擎提供。口音與離線發音取決於已安裝的語音。</Text>
          <Text style={[s.sectionTitle, textColor]}>學習資料</Text><View style={s.stack}>{button('匯出備份', () => void exportData(), true, 'download-outline')}{button('匯入備份', () => { setBackup(''); setBackupMode('import'); setInfo('backup'); }, true, 'push-outline')}{button('匯出自動保留的舊資料', () => void exportData(true), true, 'time-outline')}</View>
          <Text style={[s.sectionTitle, textColor]}>關於這個版本</Text><View style={[s.about, cardColor]}><Text style={[s.body, textColor]}>英文學習：{learningSample.length} 個自行編寫的試用詞條。{'\n'}漢英參考：{reference.entries.length} 筆 CC-CEDICT 小樣。</Text><Text style={[s.body, mutedColor]}>內容仍在測試與編校中。本版不收費，不需要建立帳號。學習紀錄保存在這台裝置，換裝置前請先匯出。</Text>{button('詞庫來源與授權', () => setInfo('sources'), true)}{button('隱私說明', () => setInfo('privacy'), true)}</View><Text style={[s.centerCaption, mutedColor]}>LexiHarbor · 0.2 詞庫試用版</Text>
        </>}
      </ScrollView>}
    ...
    <Modal visible={Boolean(selected || selectedChinese || info)} animationType="slide" onRequestClose={close}>
      <SafeAreaView style={[s.safe, { backgroundColor: colors.bg }]}><View style={s.frame}><View style={s.modalHeader}><Text style={[s.caption, mutedColor]}>{selected ? '英文學習詞條' : selectedChinese ? 'CC-CEDICT 漢英詞條' : info === 'sources' ? '詞庫來源與授權' : info === 'backup' ? '學習資料備份' : '隱私說明'}</Text><Pressable accessibilityRole="button" accessibilityLabel="關閉詳情" onPress={close} style={s.iconButton}><Icon name="close" color={colors.ink} /></Pressable></View>
        <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
          ...
          {info === 'privacy' && <><Text style={[s.title, textColor]}>你的學習紀錄</Text><Text style={[s.body, textColor]}>收藏、查詢歷史、複習和顯示偏好保存在目前裝置的本機儲存。本試用版沒有帳號、廣告追蹤或收費功能，也不主動上傳你的查詢與複習紀錄。{'\n\n'}清除瀏覽器資料或移除 App 可能使本機紀錄消失，建議定期匯出備份。匯出檔案由你自行保管。{'\n\n'}發音使用裝置或瀏覽器的語音引擎，是否連網依引擎而定。GitHub Pages 等網站主機可能處理 IP 等連線紀錄，點開外部資料連結也適用對方的政策。{'\n\n'}正式上架前會補齊開發者聯絡資訊與正式隱私政策。</Text></>}
